...
def decision_tree(train_sample, train_target, ver_dict_total, pro_name,res_path,model_path,sum,treeNum,depth):
    try:
        ...
        thismax=recall_topsum/count_topsum + precision_topsum/count_topsum - FPR_topsum/count_topsum
        if thismax > sum:
...
def machine_learning(root,data,error_pro_ver,res_path,model_path,treeNum,depth):
    ...
    for index in res:
        ...
        # if pro_name in  ["Lang","Math","Closure"]:
        #     continue
        # 测试用例
...
if __name__ =="__main__":
    ...
    for treeNum in range(140, 161, 1):
        for depth in range(65,91,1):
